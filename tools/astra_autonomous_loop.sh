...
5. 空间环境感知源头与传感器软硬件/固件协同 (Perception Grounding: Phicomm M1 / EMW3080 & zM1 Firmware/Protocol Co-Design)：
   ...
   - 物理硬件真实性与实体精简：斐讯 M1 硬件仅搭载 SHT20(温湿度)、攀藤颗粒物(PM2.5)与万胜 WZ-S(甲醛)，物理上不存在 CO2、eCO2 或 TVOC 传感器；杜绝在 Home Assistant 中暴露永远不可用或虚假的幽灵实体，从 zm1 插件中果断剔除 CO2、eCO2、TVOC 实体定义。
...
7. AI 语音全链路协同与 Voice Harness 全面板现代重塑（Voice Harness Full Surface UI/UX Overhaul & Scope Guard）：
   ...
   - 深度重塑 Voice Harness 核心控制台“概览、运行记录、测试、设置”四大面板（Lit + TypeScript，统一使用 bun 构建与测试）：
     ...
     * 第一性原理：目标与手段的本质解构（First-Principles: Decoupling Ends from Means）：
       - 牢记终极目标是家庭空间的安宁舒适、意图诚实理解、物理动作可靠生效与零认知负荷（Calmness, Reliable Grounding & Zero Cognitive Burden）；
       - 警惕把辅助“手段”（监控展板、时延微折线、打字演练、繁琐滑块）异化为虚假“目的”；
       - 手段彻底退居幕后，界面若出现只回答“系统听懂了什么”、“物理世界生效了吗”、“异常时如何克制自愈/澄清”；坚决做减法与剪除技术自嗨的视觉噪音；
